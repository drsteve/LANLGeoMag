#include <Lgm_MagModelInfo.h>

/* BAL 04-Jan-2011 modified for more cases */

int main(){


    long int            Date;
    double              UTC;
    Lgm_Vector          u, B, CurlB, CurlB_para, CurlB_perp;
    Lgm_MagModelInfo    *mInfo;
    int                 i, j;


    Date = 20050831;
    UTC  = 9.0;

    mInfo = Lgm_InitMagInfo( );
    Lgm_Set_Coord_Transforms( Date, UTC, mInfo->c );

    mInfo->Bfield = Lgm_B_TS04;
    mInfo->Bfield = Lgm_B_OP77;
    mInfo->Bfield = Lgm_B_TS04;
    mInfo->Bfield = Lgm_B_cdip;
    mInfo->Bfield = Lgm_B_T89;
    mInfo->P      = 4.1011111111111118;
    mInfo->Dst    = 7.7777777777777777;
    mInfo->By     = 3.7244444444444444;
    mInfo->Bz     = -0.12666666666666665;
    mInfo->W[0]   = 0.12244444444444445;
    mInfo->W[1]   = 0.2514;
    mInfo->W[2]   = 0.089266666666666661;
    mInfo->W[3]   = 0.047866666666666668;
    mInfo->W[4]   = 0.22586666666666666;
    mInfo->W[5]   = 1.0461333333333334;


    printf("%3s", "Kp");
    printf(" %10s", "Ux (Re)");
    printf(" %10s", "Uy (Re)");
    printf(" %10s", "Uz (Re)");
    printf(" %10s", "Bx (nT)");
    printf(" %10s", "By (nT)");
    printf(" %10s", "Bz (nT)");
    printf(" %10s", "Bmag (nT)");
    printf(" %12s", "      (\u2207\u2A2FB)x");
    printf(" %12s", "      (\u2207\u2A2FB)y");
    printf(" %12s", "      (\u2207\u2A2FB)z");
    printf(" %12s", "     (\u2207\u2A2FB)\u2225x");
    printf(" %12s", "     (\u2207\u2A2FB)\u2225y");
    printf(" %12s", "     (\u2207\u2A2FB)\u2225z");
    printf(" %12s", "     (\u2207\u2A2FB)\u27C2x");
    printf(" %12s", "     (\u2207\u2A2FB)\u27C2y");
    printf(" %12s", "     (\u2207\u2A2FB)\u27C2z\n");


    for (i=0; i<=5; i++) {
        mInfo->Kp = i;
        u.x = -6.6; u.y =  1.0;  u.z =  -1.0;
        mInfo->Bfield( &u, &B, mInfo );
        Lgm_CurlB2( &u, &CurlB, &CurlB_para, &CurlB_perp, LGM_DERIV_SIX_POINT, 1e-3, mInfo );
        printf( "%3d", mInfo->Kp);
        printf( " %10g %10g %10g", u.x, u.y, u.z );
        printf( " %10g %10g %10g", B.x, B.y, B.z );
        printf( " %10g", Lgm_Magnitude( &B ) );
        printf( " %12g %12g %12g",   CurlB.x, CurlB.y, CurlB.z );
        printf( " %12g %12g %12g",   CurlB_para.x, CurlB_para.y, CurlB_para.z );
        printf( " %12g %12g %12g\n", CurlB_perp.x, CurlB_perp.y, CurlB_perp.z );
    }

    for (j=0; j<100; j++){
        mInfo->Kp = 3;
        for (i=0; i<13; i++) {
            u.x = -1. - (double)i * 0.5;
            u.y =  1.0;  u.z =  -1.0;
            mInfo->Bfield( &u, &B, mInfo );
            Lgm_CurlB2( &u, &CurlB, &CurlB_para, &CurlB_perp, LGM_DERIV_SIX_POINT, 1e-3, mInfo );
            printf( "%3d", mInfo->Kp);
            printf( " %10g %10g %10g", u.x, u.y, u.z );
            printf( " %10g %10g %10g", B.x, B.y, B.z );
            printf( " %10g", Lgm_Magnitude( &B ) );
            printf( " %12g %12g %12g",   CurlB.x, CurlB.y, CurlB.z );
            printf( " %12g %12g %12g",   CurlB_para.x, CurlB_para.y, CurlB_para.z );
            printf( " %12g %12g %12g\n", CurlB_perp.x, CurlB_perp.y, CurlB_perp.z );
        }
    }





    Lgm_FreeMagInfo( mInfo );


    exit(0);
}
