...
int main(){
    ...
    long int            Date;
    double              UTC;
    Lgm_Vector          u, B, GradB, GradB_para, GradB_perp;
    ...
    for (i=0; i<=5; i++) {
        mInfo->Kp = i;
        u.x = -6.6; u.y =  1.0;  u.z =  -1.0;
        mInfo->Bfield( &u, &B, mInfo );
        Lgm_GradB2( &u, &GradB, &GradB_para, &GradB_perp, LGM_DERIV_SIX_POINT, 1e-3, mInfo );
        printf( "%3d", mInfo->Kp);
        printf( " %10g %10g %10g", u.x, u.y, u.z );
        printf( " %10g %10g %10g", B.x, B.y, B.z );
        printf( " %10g", Lgm_Magnitude( &B ) );
        printf( " %12g %12g %12g",   GradB.x, GradB.y, GradB.z );
        printf( " %12g %12g %12g",   GradB_para.x, GradB_para.y, GradB_para.z );
        printf( " %12g %12g %12g\n", GradB_perp.x, GradB_perp.y, GradB_perp.z );
    }
