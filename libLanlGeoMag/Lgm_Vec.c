/*! \file Lgm_Vec.c
 *
 *  \brief Routines for vector operations (e.g. dot-product, cross-product,
 * etc.)
 *
 *
 *
 ...
 */
void Lgm_VectorTripleProduct(Lgm_Vector* a,
                             Lgm_Vector* b,
                             Lgm_Vector* c,
                             Lgm_Vector* d) {
    Lgm_Vector tmp;
 ...
 *          \param[in]  u: Cartesian vector (units of whatever you used for r)
 *
 *          \param[out] A: 3-element array. A[0] set to u.x, A[1] set to u.y,
 * A[2] set to u.z.
 *
 */
 ...
 *
 *          \param[in]  A: 3-element array. A[0] set to u.x, A[1] set to u.y,
 * A[2] set to u.z.
 *
 *          \param[out] u: Cartesian vector (units of whatever you used for r)
 *
...
void Lgm_InitSlerp(Lgm_Vector* a, Lgm_Vector* b, Lgm_SlerpInfo* si) {
    ...
    w = *a;
    Lgm_ScaleVector(&w, si->CosPhi);  // w = cos(phi)*a-hat
...
/**
 *  \brief
 *      Computes Spherical Linear Interpolation (SLERP) between two unit
 * vectors. \details Spherical linear interpolation (or slerping) is a method
 * for smoothly interpolating points on a sphere (its a spherical geometry
 * version of linear interpolation).  Let \f$\hat{a}\f$ and \f$\hat{b}\f$ be
 * units vectors and \f$\phi\f$ be the angle between them. We can interpolate to
...
void Lgm_Slerp(Lgm_Vector* a,
    ...
    if (si->Phi < 1e-9) {
        w1 = *a;
        Lgm_ScaleVector(&w1, (1.0 - alpha));
        w2 = *b;
        Lgm_ScaleVector(&w2, alpha);
    } else {
        w1 = *a;
        Lgm_ScaleVector(&w1, sin((1.0 - alpha) * si->Phi) / si->SinPhi);
        w2 = *b;
        Lgm_ScaleVector(&w2, sin(alpha * si->Phi) / si->SinPhi);
    }
