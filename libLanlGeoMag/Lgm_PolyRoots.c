...
/*
 *  Routines to compute roots of quadtratic, cubic and quartic equations
 * exactly. See CRC standard math tables (Beyer) for details.
 */
 ...
 *
 */
int Lgm_QuadraticRoots(double a,
                       double b,
                       double c,
                       double complex* z1,
                       double complex* z2) {
...
int Lgm_CubicRoots(double b,
    ...
    if (D2 == 0) {
        ...
        *z1 = 2.0 * SqrtNQ * cos(Theta / 3.0) -
              b / 3.0;  // real (returned as real double z1)
        *z2 = 2.0 * SqrtNQ * cos((Theta + 2.0 * M_PI) / 3.0) -
              b / 3.0;  // real (but returned as double complex z2)
        *z3 = 2.0 * SqrtNQ * cos((Theta + 4.0 * M_PI) / 3.0) -
              b / 3.0;  // real (but returned as double complex z3)
        nReal = 3;
    }
...
int Lgm_QuarticRoots(double b,
    ...
    *z4 = 0.5 * (t - E);
    ...
    nReal = 0;
    if (fabs(cimag(*z1)) < 1e-10)
        ++nReal;
    if (fabs(cimag(*z2)) < 1e-10)
        ++nReal;
    if (fabs(cimag(*z3)) < 1e-10)
        ++nReal;
    if (fabs(cimag(*z4)) < 1e-10)
        ++nReal;
 ...
 *      \f$z^4\f$ term is assumed to be 1.
 *
 *      This routine calls Lgm_QuarticRoots() and splits the results into Real
 * and Complex roots.
 *
 *      \param[in]      b            Real coefficient of the \f$z^3\f$ term.
 *      \param[in]      c            Real coefficient of the \f$z^2\f$ term.
 ...
 *
 */
int Lgm_QuarticRootsSorted(double b,
                           double c,
                           double d,
                           double e,
                           int* nReal,
    ...
    if (cimag(z1) < 1e-16) {
        RealRoots[nr++] = creal(z1);
    } else {
        ComplexRoots[nc++] = z1;
    }
    if (cimag(z2) < 1e-16) {
        RealRoots[nr++] = creal(z2);
    } else {
        ComplexRoots[nc++] = z2;
    }
    if (cimag(z3) < 1e-16) {
        RealRoots[nr++] = creal(z3);
    } else {
        ComplexRoots[nc++] = z3;
    }
    if (cimag(z4) < 1e-16) {
        RealRoots[nr++] = creal(z4);
    } else {
        ComplexRoots[nc++] = z4;
    }
...
int Lgm_PolyRoots_Roots(double* a, int n, double* wr, double* wi) {
    ...
    while (m > 1) {
        ...
        c = a[m - 1];
        disc = b2 * b2 - c;
        if (fabs(disc) / (fabs(b2 * b2) + fabs(c)) <= DBL_EPSILON)
            disc = 0.0;
 ...
 */
void Lgm_PolyRootsDeflate(double* a,
                          int n,
                          double* b,
                          double* quad,
                          double* err) {
 ...
 */
void Lgm_PolyRoots_FindQuad(double* a,
                            int n,
                            double* b,
                            double* quad,
                            double* err,
 ...
 */
void Lgm_PolyRoots_Recurse(double* a,
                           int n,
                           double* b,
                           int m,
                           double* quad,
 ...
 *
 *          \f[ a_0 x^n + a_1 x^{n-1} + a_2 x^{n-2} + \cdots + a_{n-1} x + a_n =
 * 0 \f]
 *
 *
 *  \details
 *
 *
 *      \param[in]      a   The polynomial coefficients. There should be n+1 of
 * these. \param[in]      n   The order of the polynomial. \param[in]      z The
 * complex roots found.
 *
...
int Lgm_PolyRoots(double* a, int n, double complex* z) {
    ...
    if (a[n] == 0) {
        printf(
            "Lgm_PolyRoots(): Error! Lowest coefficient (constant term) cannot "
            "be 0.\n");
        return (0);
    }
