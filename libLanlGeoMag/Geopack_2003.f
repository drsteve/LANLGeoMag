...
c  written by N. A. Tsyganenko in 1978-1979. This version is dated April 22, 2003.
c
c
...
c  below. Please consult the documentation file geopack-2003.doc (also available from this
...
C*******************************************************************
c
      SUBROUTINE SUN (IYEAR,IDAY,IHOUR,MINUTE,ISEC,GST,SLONG,SRASN,SDEC)
	  IMPLICIT REAL*8 (A-H,O-Z)
C
...
C
      ...
      FDAY=DFLOAT(IHOUR*3600+MINUTE*60+ISEC)/86400.D0
...
c=====================================================================================
C
      SUBROUTINE RECALC (IYEAR,IDAY,IHOUR,MINUTE,ISEC)
	  IMPLICIT REAL*8 (A-H,O-Z)
C
...
C
      ...
      DT=FLOAT(IY)+FLOAT(IDAY-1)/365.25-2000.
      DO 40 N=1,66
         G(N)=G00(N)
         H(N)=H00(N)
         IF (N.GT.45) GOTO 40
...
C
      ...
      CTCL=CT0*CL0
C
      CALL SUN (IY,IDAY,IHOUR,MINUTE,ISEC,GST,SLONG,SRASN,SDEC)
C
...
C
     ...
     * -0.5+FLOAT(IHOUR*3600+MINUTE*60+ISEC)/86400.
      T=DJ/36525.
...
C
      ...
      ALPHA=(0.58-0.007*BZIMF)*(1.+0.024*DLOG(PD))
...
C  BOUNDARY POINT:
C
      CALL T96_MGNP (PD,-1.D0,XGSM,YGSM,ZGSM,XMT96,YMT96,
     * ZMT96,DIST,ID96)
C
