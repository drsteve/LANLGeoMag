...
typedef struct _CB_RH0         { double RH0; 				            } _CB_RH0;		// Its R-H-"Zero"
...
/*
 *  This structure contains the numerous parameters
 *  that are passed to EXTERN, WE dont use this now(?)
 */
typedef struct _TS04Info {

    int    IOPGEN, IOPT, IOPB, IOPR;
    int    NTOT;
    double A[70];
    double PDYN, DST, BXIMF, BYIMF, BZIMF;
    double W1, W2, W3, W4, W5, W6;
    double PS;
    double X, Y, Z;
    double BXCF, BYCF, BZCF;
    double BXT1, BYT1, BZT1;
    double BXT2, BYT2, BZT2;
    double BXSRC, BYSRC, BZSRC;
    double BXPRC, BYPRC, BZPRC;
    double BXR11, BYR11, BZR11;
    double BXR12, BYR12, BZR12;
    double BXR21, BYR21, BZR21;
    double BXR22, BYR22, BZR22;
...
_TS04Info *init_TS04Info( ) {

    _TS04Info      *TS04Info = (_TS04Info *) calloc (1, sizeof(*TS04Info) );
...
void Lgm_T04_s( int IOPT, double *PARMOD, double PS, double SINPS, double COSPS, double X, double Y, double Z, double *BX, double *BY, double *BZ) {
    ...
    Lgm_EXTERN( IOPGEN, IOPTT, IOPB, IOPR, A, 79, PDYN, DST_AST, BXIMF, BYIMF, 
...
//    static int		IEPS=3;
    ...
    double		SPS2, X2, Y2, Z2, ooR, ZR2, RRH2, RRH3, F2, F4, SPSAS2, ooCPSAS;
...
double	AP( double R, double SINT, double COST) {
    ...
    double	RRC2=3.334503403,DD2=3.079071195,P1=.02602428295,R1=8.937790598,DR1=3.327934895;
    double	DLA1=4487061833,P2=.09125832351,R2=6.243029867,DR2=1.750145910,DLA2=.4181957162;
    double	P3=.06106691992,R3=2.079908581,DR3=.6828548533;
    ...
    XK2S = 1.0-XK2;
    DL   = log(1.0/XK2S);  //CHECK was DLOG in FORTRAN - MGH
...
double	APPRC( double R, double SINT, double COST) {
    ...
    XK2S = 1.-XK2;
    DL   = log(1./XK2S); 	// CEHCK was LOG in FORTRAN
    ELK  = 1.38629436112+XK2S*(0.09666344259+XK2S*(0.03590092383+
		XK2S*(0.03742563713+XK2S*0.01451196212))) +DL*
		(0.5+XK2S*(0.12498593597+XK2S*(0.06880248576+
		XK2S*(0.03328355346+XK2S*0.00441787012))));
    ...
    XK2S = 1.-XK2;
    DL   = log(1./XK2S);   	// CHECK was LOG in FORTRAN
    ELK  = 1.38629436112+XK2S*(0.09666344259+XK2S*(0.03590092383+
          XK2S*(0.03742563713+XK2S*0.01451196212))) +DL*
          (0.5+XK2S*(0.12498593597+XK2S*(0.06880248576+
          XK2S*(0.03328355346+XK2S*0.00441787012))));
