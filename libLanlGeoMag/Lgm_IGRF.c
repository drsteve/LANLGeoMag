/*
 *     IGFC.c - Copyright (c) 1999-2006 Michael G. Henderson
 ...
 *    available versions assume that you are inputing "geodetic coordinates",
 * not geocentric coordinates. So in some other codes, your Theta and Phi are
 * assummed to be geodetic and then they are converted to geocentric, then B is
 * calculated and then the B is converted back to geodetic. This is NOT what we
 * want here. Here, I assume the input coords are in geocentric and the output
 * field is also.
 *
 *    The Earth model is the WGS84 spheroid. (Equatorial radius a=6378.137 km,
 *    Polar Radius b=6356.752 km. These are also defined in LgmCtrans.h as
 * WGS84_A and WGS84_B)
 *
...
void _Lgm_IGRF(Lgm_Vector* v, Lgm_Vector* B, Lgm_CTrans* c) {
    ...
    /*
     *  IGRF 2005 goes up to N=13, but 10 ought to be good enough for our
     * purposes?
     */
    N = 13;
     ...
     */
    {
        ...
        for (n = 1; n <= N; ++n) {
            for (m = 0; m <= n; ++m)
                f1[n][m] =
                    c->Lgm_IGRF_g[n][m] * Cmp[m] + c->Lgm_IGRF_h[n][m] * Smp[m];
        }
    }
    ...
    for (nsum = 0.0, n = 1; n <= N; ++n) {
        for (msum = 0.0, m = 0; m <= n; ++m)
            msum +=
                (c->Lgm_IGRF_g[n][m] * Smp[m] - c->Lgm_IGRF_h[n][m] * Cmp[m]) *
                m * P[n][m];
        nsum += f2[n] * msum;
    }
    B_phi = nsum / st;
...
void _Lgm_IGRF2(Lgm_Vector* v, Lgm_Vector* B, Lgm_CTrans* c) {
    ...
    P_nm1_mm1 = 1.0;
    P_nm1_m = 1.0;  // Initially these are P_0,0
    dP_nm1_mm1 = 0.0;
    dP_nm1_m = 0.0;  // Initially these are dP_0,0
    ...
    for (m = 0; m <= N; ++m) {
        for (n = 1; n <= N; ++n) {
            if (n >= m) {
                ...
                B_phi += val2 * (double)m *
                         (-c->Lgm_IGRF_g[n][m] * Smp[m] +
                          c->Lgm_IGRF_h[n][m] * Cmp[m]) *
                         P_n_m;
            }
        }
    }
...
//    { // BEGIN PARALLEL
// this granularity here is too small. Parallel version actually runs slower.
// But serially, its faster than previous versions.
#if USE_OPENMP
    // #pragma omp parallel
    // firstprivate(m,n,P_nm1_m,P_nm2_m,dP_nm1_m,dP_nm2_m,P_n_m,dP_n_m,val,val2)
    ...
    for (m = 0; m <= N; ++m) {
        ...
        for (n = m; n <= N; ++n) {
            ...
            if (n > 0) {
                ...
                // B_phi   = B_phi   + (val2 *
                // (double)m*(-c->Lgm_IGRF_g[n][m]*Smp[m] +
                // c->Lgm_IGRF_h[n][m]*Cmp[m])*P_n_m);
...
/*
 *  Same recurrence relation as _Lgm_IGRF3, but attempt at making it more
 * efficient
 */
void _Lgm_IGRF4(Lgm_Vector* v, Lgm_Vector* B, Lgm_CTrans* c) {
    double r, Theta, Phi, B_r, B_theta, B_phi;
    double st, ct, sp, cp, t, gnm, hnm, Knm;
    double val, val2, val3, Cmp_m, Smp_m, Cmp[14], Smp[14], f2[14], rinv,
        Pnn[14], dPnn[14];
    int N;
    register double P_n_m, P_nm1_m, P_nm1_mm1, P_nm2_m;
    register double dP_n_m, dP_nm1_m, dP_nm1_mm1, dP_nm2_m;
...
void Lgm_InitPnm(double ct,
    ...
    */
        ...
        for (n = 0; n <= N; ++n) {
            for (m = 0; m <= n; ++m) {
                ...
                if (m == 0) {
                    ...
                    R[n][m] =
                        sqrt(2.0 * Lgm_Factorial(NmM) / Lgm_Factorial(n + m));
                }

                if (n > m + 1) {
                    c->Lgm_IGRF_TwoNm1_Over_NmM[n][m] =
                        (double)(Nm1 + n) / (double)NmM;
                    c->Lgm_IGRF_NpMm1_Over_NmM[n][m] =
                        (double)(Nm1 + m) / (double)NmM;
                }
            }
        }
    }
    ...
    for (m = 0; m <= N; ++m) {
        ...
        if (m > 0) {
            f = 1.0;
            // are we computing the following too many times? I.e., could we be
            // more efficient?
            for (i = 1; i <= m; ++i) {
                Pmm *= f * a;
                f += 2.0;
        ...
        if (m == N) {
            ...
            if (N == (Mp1)) {
                ...
                for (n = m + 2; n <= N; ++n) {
                    ...
                    Pnm = x * c->Lgm_IGRF_TwoNm1_Over_NmM[n][m] * Pnm1m -
                          c->Lgm_IGRF_NpMm1_Over_NmM[n][m] * Pnm2m;
                    P[n][m] = Pnm * R[n][m];
...
void Lgm_InitdPnm(double P[14][14], double dP[14][14], int N, Lgm_CTrans* c) {
    ...
    if (c->Lgm_IGRF_FirstCall)
        Lgm_InitSqrtFuncs(c->Lgm_IGRF_SqrtNM1, c->Lgm_IGRF_SqrtNM2, N);
    ...
    for (n = 0; n <= N; ++n) {
        for (m = n; m >= 0; --m) {
            if (m == 0) {
                ...
                if (m == n) {
                    // dP[n][m] = 0.5* sqrt((double)(q*(n+m)*(n-m+1))) *
                    // P[n][m-1];
                    dP[n][m] = c->Lgm_IGRF_SqrtNM1[n][m] * P[n][m - 1];
                } else {
                    // dP[n][m] = 0.5*( sqrt((double)(q*(n+m)*(n-m+1))) *
                    // P[n][m-1] - sqrt((double)((n+m+1)*(n-m))) * P[n][m+1] );
                    dP[n][m] = c->Lgm_IGRF_SqrtNM1[n][m] * P[n][m - 1] -
                               c->Lgm_IGRF_SqrtNM2[n][m] * P[n][m + 1];
                }
            }
        }
    }
        ...
        for (n = 1; n <= N; n++) {
            if (m <= n) {
                if (m == 0) {
                    ...
                    S[n][m] = S[n][m - 1] * sqrt((double)(2 * (n - m + 1)) /
                                                 ((double)(n + m)));
                } else {
                    S[n][m] = S[n][m - 1] *
                              sqrt((double)(n - m + 1) / ((double)(n + m)));
                }
            }
...
void Lgm_InitIGRF(double g[14][14],
                  double h[14][14],
                  int N,
                  int Flag,
                  Lgm_CTrans* c) {
    ...
    if ((fabs(Year - c->Lgm_IGRF_OldYear) > 0.0) || Flag) {
         ...
         */
        Lx = -g[1][1] * g[2][0] +
             (g[1][1] * g[2][2] + h[1][1] * h[2][2] + g[1][0] * g[2][1]) *
                 M_SQRT_3;
        Ly = -h[1][1] * g[2][0] +
             (g[1][1] * h[2][2] - h[1][1] * g[2][2] + g[1][0] * h[2][1]) *
                 M_SQRT_3;
        Lz = 2.0 * g[1][0] * g[2][0] +
             (g[1][1] * g[2][1] + h[1][1] * h[2][1]) * M_SQRT_3;
