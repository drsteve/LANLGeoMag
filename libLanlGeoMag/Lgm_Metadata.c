...
void Lgm_metadata_initvar(Lgm_metadata_variable* var,
                          int dimension,
                          int data,
                          char* name) {
    int dm[] = {dimension};
    ...
    var->attributes = NULL;
    var->data = data;
    // Lgm_metadata_addIntAttr(var, "DIMENSION", 1, &dimension); // the & since
    // it wants a pointer
    Lgm_metadata_addStringAttr(var, "DIMENSION",
                               Lgm_metadata_intArrayToString(1, dm), 1);
}
...
char* Lgm_metadata_JSONheader(int n_vars, ...) {
    ...
    // loop over n_vars vars making them all into JSON headers
    {
        ...
        for (i = 0; i < n_vars; i++) {
            ...
            if (var_tmp->data) {
                Lgm_metadata_addIntAttr(
                    var_tmp, "START_COLUMN", 1,
                    &current_col);  // the & since it wants a pointer
    ...
    strncpy(outstr, Buffer, strlen(outstr) - 1);
    outstr[L] = '\0';
    return (outstr);
}
...
char* Lgm_metadata_toJSON(Lgm_metadata_variable* var,
    ...
    p +=
        sprintf(p, "%c  \"%s\":%s{ ", comment, var->name, Lgm_metadata_TABCHAR);
    // go through each attribute and add it to the string
    {
        ...
        for (i = 0; i < var->n_attrs; i++) {
            ...
            if ((var->attributes)[i].array)
                p += sprintf(p, "[ ");
            // add the value
            if (sscanf((var->attributes)[i].value, "%lf",
                       &tmp))  // returns 1 if the string can be a double
            ...
            if (i + 1 < var->n_attrs)
                p += sprintf(p, ",\n%c %s%s", comment, Lgm_metadata_TABCHAR,
                             Lgm_metadata_TABCHAR);
            else
                p += sprintf(p, "}");
        }
    }
...
char* Lgm_metadata_stringArrayToString(int len, char** instring) {
    ...
    // TODO(MGH): in here there is no " at the start or stop of an array, feels odd
    ...
    p +=
        sprintf(p, "\"%s", instring[i]);  // i should have the right value in it
...
int Lgm_metadata_addIntAttr(Lgm_metadata_variable* var,
                            char* name,
                            int len,
                            int* invals) {
    char* outstr;
...
int Lgm_metadata_addDoubleAttr(Lgm_metadata_variable* var,
                               char* name,
                               int len,
                               double* invals) {
    char* outstr;
...
int Lgm_metadata_addStringAttrArray(Lgm_metadata_variable* var,
                                    char* name,
                                    int len,
                                    char** invals) {
    char* outstr;
...
int Lgm_metadata_addStringAttrArray2(Lgm_metadata_variable* var,
                                     char* name,
                                     int len,
                                     ...) {
    char** outstrarr;
...
int Lgm_metadata_addStringAttr(Lgm_metadata_variable* var,
                               char* name,
                               char* value,
                               int array) {
    ...
    var->n_attrs++;
    var->attributes =
        (Lgm_metadata_attr*)calloc(var->n_attrs, sizeof(Lgm_metadata_attr));
    // put the ones back that were there
    {
        int i;
        for (i = 0; i < var->n_attrs - 1;
             i++) {  // the -1 is since we have a ++ above
            var->attributes[i].name = old_attr[i].name;
            var->attributes[i].value = old_attr[i].value;
            var->attributes[i].array = old_attr[i].array;
        }
    }
    var->attributes[var->n_attrs - 1].name =
        malloc((1 + strlen(name)) * sizeof(char));
    strcpy(var->attributes[var->n_attrs - 1].name, name);
    var->attributes[var->n_attrs - 1].value =
        malloc((1 + strlen(value)) * sizeof(char));
    strcpy(var->attributes[var->n_attrs - 1].value, value);
    var->attributes[var->n_attrs - 1].array = array;

    /* Lgm_metadata_initvar(var->start_column, var->dimension, var->name,
     * newvar); */
    free(old_attr);
