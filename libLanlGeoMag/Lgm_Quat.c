 ...
 *             LLC,  http://www.geometrictools.com/Documentation/Quaternions.pdf
 * (August 18, 2010).
 *
 *
...
/*
 *  \brief
 *      Routine determines the quaternion implied by the rotation angle and
axis.
 *  \details
 *      The Quaternion is of the form;
 *            \f[ Q = ( \sin(\theta/2) \hat{u}, \cos(\theta/2) ) = (
\sin(\theta/2) u_x, \sin(\theta/2) u_y, \sin(\theta/2) u_z, \cos(\theta/2) )
\f].
 *
...
void Lgm_QuatRotateVector(double* Q, Lgm_Vector* v, Lgm_Vector* vp) {
    ...
    vp->x = ww * v->x + 2.0 * yw * v->z - 2.0 * zw * v->y + xx * v->x +
            2 * xy * v->y + 2.0 * xz * v->z - zz * v->x - yy * v->x;
    vp->y = 2.0 * xy * v->x + yy * v->y + 2.0 * yz * v->z + 2.0 * zw * v->x -
            zz * v->y + ww * v->y - 2.0 * xw * v->z - xx * v->y;
    vp->z = 2.0 * xz * v->x + 2.0 * yz * v->y + zz * v->z - 2.0 * yw * v->x -
            yy * v->z + 2.0 * xw * v->y - xx * v->z + ww * v->z;
...
void Lgm_QuatVecNormalize(double* v) {
    Lgm_QuatVecScale(v, 1.0 / Lgm_QuatVecLength(v));
}
 ...
 */
void Lgm_QuatSquad(double Q0[4],
                   double Q1[4],
                   double S0[4],
                   double S1[4],
                   double h,
...
void Lgm_QuatSquadComputeAuxPoint(double Qim1[4],
                                  double Qi[4],
                                  double Qip1[4],
                                  double si[4]) {
    ...
    Lgm_QuatMultiply(Qi_inv, Qip1, A);
    Lgm_QuatLog(A, logA);
    Lgm_QuatMultiply(Qi_inv, Qim1, B);
    Lgm_QuatLog(B, logB);
...
/**
 *  \brief
 *      Computes Spherical Spline Quaternion Interpolation (SQUAD) on arrays of
 * quaternions. \details Spherical spline interpolation is a method for smoothly
 *      interpolating points on a sphere.
 *
 ...
 *
 */
int Lgm_QuatSquadInterp(double* T,
                        double* Q[4],
                        long int N,
                        double* t,
                        double* q[4],
    ...
    if (N < 2) {
        printf(
            "Lgm_QuatSquadInterp: Not enough points to interpolate: N = %ld\n",
            N);
        return (-1);

    } else if (N == 2) {
        printf(
            "Lgm_QuatSquadInterp: Not enough points to perform a SQUAD "
            "interpolation: N = %ld. Do a SLERP instead\n",
            N);
    ...
    for (j = 0; j < n; j++) {
        ...
        if (i < 0) {
            printf(
                "Lgm_QuatSquadInterp: Warning. Extrapolation required on the "
                "low side. t[%ld] = %g. i = %ld\n",
                j, t[j], i);
            return (-1);
            ...
            printf(
                "Lgm_QuatSquadInterp: Warning. Extrapolation required on the "
                "high side. t[%ld] = %g. i = %ld\n",
                j, t[j], i);
            return (-1);

        } else if (i == 0) {
            printf(
                "Lgm_QuatSquadInterp: Warning. i == 0, can only do SLERP on "
                "[0,1] interval.\n");
            ip1 = i + 1;
            h = (t[j] - T[i]) /
                (T[ip1] - T[i]);  // fraction of the way through the interval --
                                  // is there abtter way to do this than linear?
            if ((h < 0.0) || (h > 1.0)) {
                printf(
                    "h not in range [0-1], h = %g   j=%ld i=%ld t[j] = %g T[i] "
                    "= %g T[i+1] = %g\n",
                    h, j, i, t[j], T[i], T[i + 1]);
                // exit(0);
            }
            ...
            printf(
                "Lgm_QuatSquadInterp: Warning. i == %ld, can only do SLERP on "
                "[%ld,%ld] interval.\n",
                i, N - 2, N - 1);
            ip1 = i + 1;
            h = (t[j] - T[i]) /
