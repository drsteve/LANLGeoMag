 ...
 * References: (1) Tsyganenko, N. A., and V. A. Andreeva (2016), "An empirical
 *RBF model of the magnetosphere parameterized by interplanetary and
 *ground-based drivers", v.121, doi:10.1002/2016JA023217, accepted by JGRA,
 *10/17/2016. (2) Andreeva, V. A., and N. A. Tsyganenko (2016), "Reconstructing
 *the magnetosphere from data using radial basis functions, JGRA Space Physics,
 *v.121, 2249-2263, doi:10.1002/2015JA022242. ORIGINAL CODE BY: N. A. Tsyganenko
 *AND V. A. Andreeva
 *------------------------------------------------------------------------------------------------------
 * NOTE: THE MODEL IS VALID ONLY UP TO Xgsw=-15 Re and should NOT be used
 *tailward of that distance
 *------------------------------------------------------------------------------------------------------
 */
#ifdef HAVE_CONFIG_H
#include "config.h"
...
static double Lgm_TA16_As[22] = {
    12.544,  -0.194, 0.305,  0.0573, 2.178,  0.0571, -0.999, 16.473,
    0.00152, 0.382,  0.0,    0.0,    -0.210, 0.0,    0.0,    -0.636,
    -2.600,  0.832,  -5.328, 1.103,  -0.907, 1.450};
...
int Lgm_SetCoeffs_TA16(long int Date, double UTC, LgmTA16_Info* ta) {
    ...
    int idx, ncols;
    FILE* fp;
    char *Path, TA16_path[1024], datafile[2048], tmpstr[1300];
    ...
    ta->Xind_avg =
        1.;  // Avg. of coupling param based on Newell, over previous 30 minutes
    ta->By_avg = 3.;  // Avg. over previous 30 minutes of By

    // If flag set, calculate from Qin-Denton...
    if (!(ta->readTAparams)) {
        // NOT CURRENTLY IMPLEMENTED - just use the defaults set above and skip
        // to the end
        printf(
            "Lgm_SetCoeffs_TA16: Calculating input parameters from QD data is "
            "not currently supported. Setting default values.\n");
        return (0);
        ...
        for (idx = 0; idx < qin->nPnts; idx++) {
            ...
            bt = sqrt(bz2 +
                      by2);  // QD files don't have Bx, so just use transverse
            ...
            // TODO(SKM): take window averages for given time
        }
        // Clean up
        ...
        if (Path == NULL) {  // setting path
            ...
            if (access(TA16_path, F_OK) < 0) {
                printf(
                    "Lgm_SetCoeffs_TA16: Warning, TA16 Data directory not "
                    "found at %s. Use TA16_DATA_PATH environment variable to "
                    "set path.\n",
                    TA16_path);
                exit(-1);
            } else {
        ...
        // <Bx IMF>    f8.2     nT, in GSW coordinates, average over 30-min
        // trailing interval <By IMF>    f8.2     nT, in GSW coordinates,
        // average over 30-min trailing interval <Bz IMF>    f8.2     nT, in GSW
        // coordinates, average over 30-min trailing interval Vx          f8.1
        // solar wind Vx in GSE coordinates, km/s Vy          f8.1     solar
        // wind Vy in GSE coordinates, km/s Vz          f8.1     solar wind Vz
        // in GSE coordinates, km/s Np          f7.2     solar wind proton
        // density, cm^-3 T           f9.0     solar wind proton temperature,
        // degs K Sym-H       f7.1     Sym-H index, nT IMF flag    i5 equals 1
        // for actually measured or 2 for linearly interpolated IMF SW flag i5
        // equals 1 for actually measured or 2 for linearly interpolated solar
        // wind parameters Tilt        f8.4     geodipole tilt angle (radians!)
        // in GSW coordinate system Pdyn        f7.2     solar wind flow ram
        // pressure, nPa <N-index>   f8.4     average over 30-min trailing
        // interval, see Eq.(1) in TA16_Model_description.pdf <B-index>   f8.4
        // average over 30-min trailing interval, see Eq.(2) in
        // TA16_Model_description.pdf <SymHc>     f7.1     sliding average over
        // 30-min interval, centered on the current time moment
        sprintf(datafile, "%s/%ld_OMNI_5m_with_RBF_TA16_drivers.dat", TA16_path,
                Date / 10000);
        if ((fp = fopen(datafile, "r")) != NULL) {
            ...
            while (fgets(tmpstr, 1300, fp) != NULL) {
                ncols = sscanf(tmpstr,
                               ...
                               "%lf %d %d %lf %lf %lf %lf %lf",
                               &year, &doy, &hour, &minute, &bx_av, &by_av,
                               &bz_av, &vx, &vy, &vz, &nden, &temp, &symh,
                               &IMFflag, &SWflag, &tilt, &pdyn, &newe_avg,
                               &boyn_avg, &symhc_avg);
                lineutc = (double)hour + (double)minute / 60.0;
                if ((abs(doy - indoy) == 0) &&
                    (fabs(lineutc - UTC) < fivemin / 2.0)) {
                    // nearest time, so fill ta structurea
                    match = 1;
                    if (ta->verbosity > 0)
                        printf("Date/Time: %ld %lf\nUsing %s", Date, UTC,
                               tmpstr);
                    ta->Pdyn = pdyn;
            ...
            printf(
                "Lgm_SetCoeffs_TA16(): Line %d in file %s. Could not open file "
        ...
        if (!(match))
            printf(
                "Lgm_SetCoeffs_TA16: Warning, No nearby times found in %s. "
...
int TA2016_SetGrid(LgmTA16_Info* Info) {
    ...
    Klat = 8;          // Klat IS THE NUMBER OF LATITUDE CIRCLES IN THE NORTHERN
                       // HEMISPHERE (EXCLUDING THE POLE)
    ...
    Nlat = Klat + 1;  // NUMBER OF LATITUDE CIRCLES IN THE N. HEMISPHERE
                      // INCLUDING THE POLE

    R = RLowGrid;  //  THE INNERMOST SPHERE RADIUS

    PD_TR = 0.5;  //  this is just to filter out RBF centers outside the Lin et
                  //  al model magnetopause
    ...
    for (J = 1; J <= 100;
         J++) {  // J COUNTS THE NUMBER OF SPHERES WITH RBF CENTERS
                 // (WHICH IS ACTUALLY MUCH LESS THAN 100)
        for (I = 1; I <= Nlat; I++) {  // I COUNTS THE LATITUDE CIRCLES (FROM
                                       // NORTH POLE DOWN TO EQUATOR)
            XcolatD =
                (90.0 / (Nlat - 0.5)) *
                ((double)(I)-1.0);  //  COLATITUDE OF THE Ith CIRCLE (in degs)
            Nlon = 4 * (I - 1);     //  NUMBER OF RBF CENTERS ON THE ItH CIRCLE
            if (I != 1) {
                dLonDeg =
                    360.0 / Nlon;  //  LONGITUDE INTERVAL BETWEEN CENTERS (degs)
            } else {
            ...
            for (K = 1; K <= Nlon; K++) {
                ...
                Xlon =
                    XlonD * 0.01745329252;  // LONGITUDE OF THE Lth RBF CENTER
                ...
                ZZZZ = R * cos(Xcolat);
                // HERE WE CALCULATE THE DISTANCE FROM THE RBF NODE TO THE LIN
                // ET AL. [2010] MODEL MAGNETOPAUSE. THE RBF NODES LYING OUTSIDE
                // THE BOUNDARY ARE NOT INCLUDED IN THE GRID
                EN = Lgm_TA16_As[21];
                ES = Lgm_TA16_As[21];
                ...
                T = atan2(STT, CTT);
                if (Rho1 > 1.0e-5) {  // WE ARE NOT ON THE X-AXIS - NO
                    ...
                    if (XXXX > 0.0) {  // IF ON THE DAYSIDE, NO PROBLEM EITHER
                        ...
                        RM = 1000.0;  // ASSIGN RM=1000 (A CONVENTIONAL
                                      // SUBSTITUTE VALUE)
                ...
                B0 = Lgm_TA16_As[6];
                F1 = pow(sqrt(0.5 * (1.0 + CTT)) +
                             Lgm_TA16_As[5] * 2.0 * STT * CTT * (1.0 - exp(-T)),
                         B0);
                R0 = Lgm_TA16_As[0] * pow(PD_TR + PM, Lgm_TA16_As[1]);
                RM = R0 * F1 + CN * exp(DN * pow(PSIN, EN)) +
                     CS * exp(DS *
                              pow(PSIS,
                                  ES));  // POSITION OF THE MODEL MAGNETOPAUSE
                if (R <= RM) {
                    ...
                    Info->RHBR[L] =
                        RH / R *
                        (1.0 - pow(1.0 + pow(R / RH, Alpha), 1.0 / Alpha));
                }
            }  // End loop over K
        }      // End loop over I

        RLAST = R;
        R = R * (Nlat - 0.5 + M_PI / 4.0) /
            (Nlat - 0.5 - M_PI / 4.0);  // Increment R by a fixed factor
        if (R > RHighGrid)
            break;  // RBF centers only inside R=RHIGH
    }               // End loop over J
    Info->SetupDone = TRUE;
    return (1);
}

int TA2016(Lgm_Vector* posGSM,
           double* PARMOD,
           Lgm_CTrans* ctrans,
           Lgm_Vector* BvecGSM,
           LgmTA16_Info* Info) {
    ...
    posSM.x = posGSM->x * cPS -
              posGSM->z * sPS;  //  RBF EXPANSIONS ARE IN SM COORDINATES
    ...
    for (I = 1; I <= 1296; I++) {
        ...
        CP = sqrt(dcpdenx * dcpdenx + dcpdeny * dcpdeny + dcpdenz * dcpdenz +
                  D2);  // RBF Ch_i+
        CM = sqrt(dcmdenx * dcmdenx + dcmdeny * dcmdeny + dcmdenz * dcmdenz +
                  D2);  // RBF Ch_i-
        ...
        PCP.x = 2.0 * DCP.x - posSM.x * (DCPsq.y + DCPsq.z) + posSM.y * DCPXY +
                posSM.z * DCPXZ;
        PCP.y = 2.0 * DCP.y - posSM.y * (DCPsq.x + DCPsq.z) + posSM.z * DCPYZ +
                posSM.x * DCPXY;
        PCP.z = 2.0 * DCP.z - posSM.z * (DCPsq.x + DCPsq.y) + posSM.x * DCPXZ +
                posSM.y * DCPYZ;
        PCM.x = 2.0 * DCM.x - posSM.x * (DCMsq.y + DCMsq.z) + posSM.y * DCMXY +
                posSM.z * DCMXZ;
        PCM.y = 2.0 * DCM.y - posSM.y * (DCMsq.x + DCMsq.z) + posSM.z * DCMYZ +
                posSM.x * DCMXY;
        PCM.z = 2.0 * DCM.z - posSM.z * (DCMsq.x + DCMsq.y) + posSM.x * DCMXZ +
                posSM.y * DCMYZ;
        ...
        // -----------------   TOTAL FIELD: -----------------------------------
        ACT = Info->A[I] + Info->A[I + 5184] * FPD + Info->A[I + 10368] * SymV +
              Info->A[I + 15552] * Xind;
        AST = Info->A[I + 1296] + Info->A[I + 6480] * FPD +
              Info->A[I + 11664] * SymV + Info->A[I + 16848] * Xind;
        AT = Info->A[I + 20736] * ByIMF;
        ACP = Info->A[I + 2592] + Info->A[I + 7776] * FPD +
              Info->A[I + 12960] * SymV + Info->A[I + 18144] * Xind;
        ASP = Info->A[I + 3888] + Info->A[I + 9072] * FPD +
              Info->A[I + 14256] * SymV + Info->A[I + 19440] * Xind;
        AP = Info->A[I + 22032] * ByIMF;
        BvecSM.x += CTarr.x * ACT + STarr.x * AST + (TCP.x - TCM.x) * AT +
                    CParr.x * ACP + SParr.x * ASP + (PCP.x + PCM.x) * AP;
        BvecSM.y += CTarr.y * ACT + STarr.y * AST + (TCP.y - TCM.y) * AT +
                    CParr.y * ACP + SParr.y * ASP + (PCP.y + PCM.y) * AP;
        BvecSM.z += CTarr.z * ACT + STarr.z * AST + (TCP.z - TCM.z) * AT +
                    CParr.z * ACP + SParr.z * ASP + (PCP.z + PCM.z) * AP;
    }
